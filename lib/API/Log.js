  ...
  Client.launchBus(function (err, bus, socket) {
    socket.on('reconnect attempt', function () {
      if (global._auto_exit === true) {
        if (timestamp) { process.stdout.write(chalk['dim'](chalk.grey(moment().format(timestamp) + ' '))); }
    ...
    bus.on('log:*', function (type, packet) {
      ...
      lines.forEach(function (line) {
        ...
        } else if (!raw && (id === 'all' || id === 'PM2')) { process.stdout.write(chalk.blue(pad(DEFAULT_PADDING, 'PM2') + ' | ')); }
