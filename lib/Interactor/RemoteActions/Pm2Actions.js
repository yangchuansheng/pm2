...
var Pm2Actions = module.exports = {
  ...
  pm2Actions: function () {
    ...
    this.socket.data('trigger:pm2:action', function (raw_msg) {
      ...
      d.run(function () {
        if (!msg) { throw new Error('Wrong SECRET KEY to uncipher package'); }
  ...
  pm2ScopedActions: function () {
    ...
    function sendEvent (event, data) {
      ...
      if (process.env.NODE_ENV == 'local_test') { process.send({event: event, data: data}); }
    }
    ...
    function executionBox (msg, cb) {
      ...
      if (action_conf.lock === false) { opts.lock = false; }
