...
var TransactionAggregator = module.exports = function (pushInteractor) {
  ...
  this.aggregate = function (packet) {
    ...
    if (!new_trace.spans[0].labels) { return log('Trace spans without labels: %s', Object.keys(new_trace.spans)); }

    if (!self.processes[packet.process.name]) { self.processes[packet.process.name] = initializeRouteMeta(packet.process); }
  ...
  this.mergeTrace = function (aggregated, trace) {
    ...
    } else { aggregated.meta.mean = trace.spans[0].mean; }
    ...
    for (var i = 0, len = keys.length; i < len; i++) {
      ...
      for (var j = path.length - 1; j >= 0; j--) {
        ...
        if (path[j] !== segments[j]) {
          ...
          if (self.isIdentifier(path[j]) && segments[j] === '*' && path[j - 1] === segments[j - 1]) { return segments.join('/'); }
  ...
  this.isIdentifier = function (id) {
    ...
    else if (id.match(/[0-9]+[a-z]+|[a-z]+[0-9]+/)) { return true; } else { return false; }
  };
