...
var PushInteractor = module.exports = {
  ...
  sendFile: function (packet) {
    ...
    if (packet.data.return.heapdump === true) { meta.heapdump = true; }
    ...
    if (packet.process && !packet.server) {
      if (event === 'logs'
          && (JSON.stringify(self._packet[event]).length > logs_limit_size
              || self._packet[event].length > 100)) { return console.error('Logs packet larger than 50KB limit'); }
  ...
  sendData: function () {
    ...
    this.preparePacket(function () {
      ...
      self.socket.client.sendv2(str, function () {
        ...
        if (duration_sec > 1) { console.info('[WARN] Time to send data over TCP took %dseconds!', duration_sec); }
