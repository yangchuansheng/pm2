...
Common.prepareAppConf = function (opts, app) {
   ...
   */
  if (cst.PM2_PROGRAMMATIC) { Common.safeExtend(env, process.env); } else { env = process.env; }
...
Common.sink.resolveInterpreter = function (app) {
  var noInterpreter = (!app.exec_interpreter || app.exec_interpreter == 'none'),
  ...
  // Else if no Interpreter detect if process is binary
    ...
    if (!process.env.NVM_DIR) {
      ...
      } else { nvm_node_path = path.join(process.env.NVM_DIR, '/v' + node_version + '/bin/node'); }
...
function checkExecMode (conf) {
  ...
  if (conf.instances && conf.exec_mode === undefined) { conf.exec_mode = 'cluster_mode'; }
...
function checkDeprecates (conf) {
  ...
  if (typeof (conf.instances) === 'string') { conf.instances = parseInt(conf.instances) || 0; }
}
