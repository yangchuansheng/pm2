    ...
    async.eachLimit(processes, cst.CONCURRENT_ACTIONS, function (proc, next) {
      ...
      if (proc.pm2_env.status === cst.ONLINE_STATUS) {
        return God.restartProcessId({id: proc.pm2_env.pm_id}, next);
               ...
               && proc.pm2_env.status !== cst.LAUNCHING_STATUS) { return God.startProcessId(proc.pm2_env.pm_id, next); } else { return next("[Watch] Process name %s is being stopped so I won't restart it", name); }
    }, function (err) {
      if (err) return cb(God.logAndGenerateError(err));
      return cb(null, God.getFormatedProcesses());
    });
